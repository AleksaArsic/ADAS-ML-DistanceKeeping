...
output_path = './model_out/model_out_center_it5_b2_200_200_addition_1_two_loss/' # output folder to save results of training
...
loadSize = 2438          # how much images and labels to load
...
# reshape labels to two input arrays to fit output of the defined multi-output model
def reshapeLabels(labels):
    labels_temp = np.transpose(labels)

    labels_last = labels_temp[-1]
    # labels_last is 1-D array, make it 2-D and transpose
    labels_last = np.array(labels_last)[np.newaxis]
    labels_last = np.transpose(labels_last)

    labels_temp = np.transpose(np.delete(labels_temp, -1, 0))
    
    return [labels_temp, labels_last]

# shape labels for writing to .csv file
def shapeLabels(labels):
    tempLab1 = np.transpose(labels[0])
    tempLab2 = np.transpose(labels[1])

    lab = np.concatenate((tempLab1, tempLab2))
...
if __name__ == '__main__':
    ...
    # create CNN model
    model = create_cnn_model(in_width, in_heigth, in_channels, output_no, output_path)
    ...
    # reshape labels to fit outputs of defined multi-output model
    tr_cat = reshapeLabels(tr_cat)
    val_labels = reshapeLabels(val_labels)
    df_labels = reshapeLabels(df_labels)
    ...
    # define callbacks
    callbacks = [
        EarlyStopping(monitor='val_veh_pred_cat_acc', mode = 'max', patience=30, verbose=1),
        ReduceLROnPlateau(monitor='val_veh_pred_cat_acc', mode = 'max', factor=0.0025, patience=10, min_lr=0.000001, verbose=1),
        ModelCheckpoint(model_out_path, monitor='val_veh_pred_cat_acc', mode = 'max', verbose=1, save_best_only=True, save_weights_only=False),
    ...
    val_acc = history_dict['val_veh_pred_cat_acc']
    val_loss = history_dict['val_loss']
    train_acc = history_dict['veh_pred_cat_acc']
    ...
    # shape data for expected output for writing to .csv
    predictions = shapeLabels(predictions)
